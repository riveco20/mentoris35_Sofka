...
public class RandomController {
    ...
    @PostMapping("/numberRamdo")
    public Mono<Random> postNumberRamdo(@RequestBody RequestDTO request) {
        return Mono.just(new Random()).map(entity -> {
            entity.setDate(new Date());
            entity.setOrginalList(IntStream.range(request.getNumberInitial(), request.getNumberFinal()+1)
